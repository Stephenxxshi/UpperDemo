...
public class MultiFormatConfigLoader
{
    ...
    public List<T> LoadFromDirectory<T>(string directoryPath, string searchPattern = "*.*")
    {
        ...
        foreach (var file in files)
        {
            try
            {
                ...
                _logger.LogInformation($"[ 加载配置服务 ] 已从 {Path.GetFileName(file)} 读取 {items.Count} 条数据");
            }
            catch (Exception ex)
            {
    ...
    public List<T> LoadFromFile<T>(string filePath)
    {
        ...
        {
            _logger.LogWarning($"[ 加载配置服务 ]未找到文件: {filePath}");
            return new List<T>();
        }
        ...
        {
            _logger.LogError(ex, $"[ 加载配置服务 ]  解析配置文件失败: {filePath}");
            return new List<T>();
        }
    }
}
